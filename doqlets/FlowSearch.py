...
# query_responder_llm = LLM("LLM", config={"api_key": api_key, "model":"gpt-5-mini"})
...
searchFlow.add_node(semanticSearchNode)
searchFlow.add_node(mergeSearchResultsNode)

searchFlow.add_edge("START","SemanticSearch|semantic_search")
searchFlow.add_edge("SemanticSearch|semantic_search","MergeSearchResults|merge_search_results")
searchFlow.add_edge("MergeSearchResults|merge_search_results","END")

searchFlow.add_data_source("SemanticSearch|semantic_search|query", "INPUTS|query")

searchFlow.add_data_source("MergeSearchResults|merge_search_results|results", "SemanticSearch|semantic_search|results")

searchFlow.set_outputs(["MergeSearchResults|merge_search_results|merged_results"])


#searchFlow.run({"query": "What are LLMs?"})